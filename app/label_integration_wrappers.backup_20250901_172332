...
class LabelGenerator:
    ...
    def generate_labels_for_text(self, text: str, metadata: Optional[Dict] = None) -> List[Dict]:
        ...
        Returns:
            List of label dictionaries with 'label' and 'confidence' keys
        """
        ...
        try:
            ...
            # Flatten the categorized labels into a single list
            labels = []
            
            for category in ["topic", "tone", "intent"]:
                category_labels = label_generator._norm_list(payload.get(category, []))
